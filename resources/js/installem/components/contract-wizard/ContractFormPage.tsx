      ...
      <FormCard>
        <FormSection icon={<FileText className="h-4 w-4" />} title="Contract Identity" description="Origin, ownership and prioritisation.">
          ...
          <FormRowDouble
            left={{ label: "Salesperson", required: true, children: (
              <WSelect value={salesPerson} onChange={setSalesPerson} options={salesTeam.map((s: any) => s.name)} placeholder="Assign sales…" />
            )}}
            right={{ label: "Recovery Agent", hint: "Owns collection for this contract", children: (
              <WSelect value={recoveryAgent} onChange={setRecoveryAgent} options={agents.map((a: any) => a.name)} placeholder="Assign agent…" />
            )}}
          />
        </FormSection>
        ...
        <FormSection icon={<Package className="h-4 w-4" />} title="Product & Financing" description="Pick the item, choose a linked installment plan, set the down payment.">
          <FormRow label="Product" required>
            <WSelect value={product?.name || ""} onChange={(name) => { const p = products.find((x: any) => x.name === name); setProductId(p?.id || ""); setVariantSku(""); }} options={products.map((p: any) => p.name)} placeholder="Select product…" />
          </FormRow>
          <FormRow label="Variant & Qty" hint={variants.length === 0 ? "No variants" : undefined}>
            <FieldPair>
              <WSelect value={variant?.name || variant?.sku || ""} onChange={(v) => { const found = variants.find((x: any) => (x.name || x.sku) === v); setVariantSku(found?.sku || ""); }} options={variants.map((v: any) => v.name || v.sku)} placeholder={variants.length === 0 ? "—" : "Select variant…"} />
                ...
                {availablePlans.map((p: any) => {
                  ...
                  return (
                    <button key={p.id} type="button" onClick={() => setPlanId(p.id)}
                      ...
                      <div className="text-[11px] text-muted-foreground mt-0.5">Markup {p.markup}% • DP {p.downPayment}% suggested</div>
                    </button>
                  );
                })}
              </div>
          ...
          />
          <FormRow label="Down Payment (Rs.)" required hint={cashPrice > 0 ? `Range ${Rs(minDown)} — ${Rs(maxDown)} (75% max) • ${cashPrice ? ((down / cashPrice) * 100).toFixed(1) : 0}% of cash price` : "Select product & plan to set down payment"}>
          ...
          {plan && cashPrice > 0 && (
            <FormRowFull tone="muted">
              ...
              <div className="text-[13px] grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-x-4 gap-y-1.5">
                ...
                <KV k={`${frequency} EMI × ${tenure}`} v={Rs(monthly)} accent />
              </div>
            </FormRowFull>
          )}
        </FormSection>

        <FormSection icon={<CalendarDays className="h-4 w-4" />} title="Installment Schedule" description={schedule.length ? `${tenure} installments · ${frequency}` : "Auto-generated once product & plan are selected"}>
          <FormRowFull>
          ...
          {guarantorList.map((g, i) => (
            <FormRowFull key={i} tone={i % 2 === 1 ? "muted" : undefined}>
              <div className="flex items-center justify-between mb-3">
                <div className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Guarantor {i + 1}{i === 0 && <span className="ml-2 text-primary normal-case">• Primary</span>}</div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-3">
                ...
                <Mini label="CNIC" required={i === 0}><WInput value={g.cnic} onChange={(e) => updateGuarantor(i, { cnic: e.target.value })} placeholder="35202-…" /></Mini>
      ...
      <div className="fixed bottom-0 left-0 right-0 lg:left-[var(--sidebar-w,16rem)] z-30 border-t border-border bg-background/95 backdrop-blur px-4 sm:px-6 py-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3 flex-wrap">
          <div className="text-xs">
            {error ? <span className="text-destructive font-semibold flex items-center gap-1.5"><AlertTriangle className="h-3.5 w-3.5" /> {error}</span> : <span className="text-muted-foreground">Will be saved as <strong className="text-foreground">Under Process</strong>{plan ? ` · ${tenure} × ${Rs(monthly)}` : ""}.</span>}
          </div>
  ...
  return (
    <div ref={wrapRef} className="relative">
      <button type="button" onClick={() => setOpen((o) => !o)}
        ...
        <span className={value ? "text-foreground" : "text-muted-foreground"}>{value || "Select customer…"}</span>
