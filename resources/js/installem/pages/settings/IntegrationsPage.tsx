...
import { PageHeader, StatCard } from "@/shared/components/shared/UiKit";
import { UnderlineTab, UnderlineTabBar } from "@/shared/components/ui/underline-tabs";
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((it) => {
            ...
            return (
              <button key={it.id} type="button" onClick={() => setActive(it)} className={`group relative text-left rounded-xl border border-border bg-card overflow-hidden transition-all hover:-translate-y-0.5 hover:shadow-lg hover:shadow-primary/5 hover:border-primary/30 ${on ? `ring-1 ${tc.ring}` : ""}`}>
